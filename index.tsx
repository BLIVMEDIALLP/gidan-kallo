...
function TurfPage({ setPage, onBook }) {
  ...
  return (
    <div style={{paddingBottom:100}}>
      ...
      {/* CTA */}
      {selSlot!==null && (
        <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,padding:"12px 16px 24px",background:"linear-gradient(to top,#09090b,#09090b 70%,transparent)",zIndex:40}}>
...
function FoodPage({ setPage, cart, setCart }) {
  ...
  return (
    <div style={{paddingBottom:100}}>
      ...
      <div style={{padding:"16px",display:"flex",alignItems:"center",gap:12}}>
        <div style={{flex:1}}>
          <div style={{color:"#9ca3af",fontSize:11,fontWeight:700,letterSpacing:1}}>GIDAN KALLO</div>
          <div style={{color:"#FFD700",fontWeight:700,fontSize:22}}>Food &amp; Drinks</div>
          <div style={{color:"#6b7280",fontSize:12,marginTop:2}}>Fresh • Flavorful • Cinema-Ready 🎬</div>
        </div>
        {/* Cart button */}
      ...
      {totalQty>0&&(
        <div style={{position:"fixed",bottom:70,left:"50%",transform:"translateX(-50%)",width:"calc(100% - 32px)",maxWidth:398,zIndex:40}}>
...
function Nav({ page, setPage }) {
  ...
  return (
    <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,background:"#18181b",borderTop:"1px solid #27272a",display:"flex",zIndex:50}}>
      {tabs.map(t=>(
        <button key={t.id} onClick={()=>setPage(t.id)} style={{flex:1,padding:"8px 0 10px",background:"none",border:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
          <span style={{fontSize:20,lineHeight:1}}>{t.emoji}</span>
          <span style={{fontSize:10,color:page===t.id?"#FFD700":"#6b7280",fontWeight:page===t.id?700:400}}>{t.label}</span>
        </button>
      ))}
    </div>
  );
  ...
  return (
    <div style={{paddingBottom:100}}>
      <div style={{padding:"16px",display:"flex",alignItems:"center",gap:12}}>
        ...
        <div><div style={{color:"#9ca3af",fontSize:11}}>ALL FEEDBACK</div><div style={{color:"white",fontWeight:700,fontSize:20}}>Customer Reviews</div></div>
      </div>
      <div style={{padding:"0 16px"}}>
        {reviews.map((r,i)=>(
          <div key={i} style={{background:"#18181b",border:"1px solid #27272a",borderRadius:14,padding:14,marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <div style={{background:r.color,borderRadius:"50%",width:38,height:38,display:"flex",alignItems:"center",justifyContent:"center",color:"white",fontWeight:700,fontSize:13,flexShrink:0}}>{r.initials}</div>
                <div><div style={{color:"white",fontWeight:700,fontSize:14,marginBottom:4}}>{r.name}</div><div style={{display:"flex",gap:6}}><Stars count={r.rating} size={13}/><CategoryBadge cat={r.category}/></div></div>
              </div>
              <span style={{color:"#6b7280",fontSize:11,flexShrink:0}}>{r.when}</span>
            </div>
            <div style={{color:"#9ca3af",fontSize:13,lineHeight:1.5}}>{r.text}</div>
          </div>
        ))}
      </div>
      <div style={{position:"fixed",bottom:72,right:16,zIndex:40}}>
  ...
  return (
    <div style={{paddingBottom:80}}>
      <div style={{padding:"16px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <GKLogo size={44}/>
          <div><div style={{color:"#9ca3af",fontSize:11,letterSpacing:1}}>WELCOME BACK</div><div style={{color:"#FFD700",fontWeight:700,fontSize:16}}>Gidan Kallo</div></div>
        </div>
      ...
      <div style={{margin:"0 16px",borderRadius:18,overflow:"hidden",position:"relative",height:260}}>
        <div style={{width:"100%",height:"100%",background:hero.gradient,position:"relative"}}>
          <div style={{position:"absolute",top:-40,right:-40,width:200,height:200,borderRadius:"50%",background:"rgba(255,255,255,0.04)"}}/>
          <div style={{position:"absolute",right:20,top:"50%",transform:"translateY(-60%)",fontSize:140,fontWeight:900,color:"rgba(255,255,255,0.07)",fontFamily:"serif",lineHeight:1,userSelect:"none"}}>{hero.posterLetter}</div>
          <div style={{position:"absolute",inset:0,background:"linear-gradient(to top,rgba(0,0,0,0.9) 0%,rgba(0,0,0,0.1) 55%,transparent 100%)"}}>
            ...
            <div style={{position:"absolute",top:12,right:12,background:hero.badgeColor,color:"white",fontSize:10,fontWeight:700,padding:"3px 10px",borderRadius:6}}>{hero.badge}</div>
            <div style={{position:"absolute",bottom:76,left:14,display:"flex",gap:5}}>
            ...
            <div style={{position:"absolute",bottom:12,left:14,right:14}}>
              <div style={{color:"white",fontWeight:700,fontSize:22,lineHeight:1.2,marginBottom:4}}>{hero.title}</div>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:10}}><Stars count={Math.floor(hero.rating)} size={13}/><span style={{color:"#9ca3af",fontSize:12}}>• {hero.duration} • {hero.genre}</span></div>
              <button onClick={()=>setPage("cinema")} style={{width:"100%",padding:"11px",background:"white",color:"black",border:"none",borderRadius:10,fontWeight:700,fontSize:14,cursor:"pointer"}}>🎟 Get Tickets</button>
            </div>
          </div>
        </div>
      </div>
      <div style={{padding:"16px"}}>
        <div style={{color:"white",fontWeight:700,fontSize:16,marginBottom:12}}>Quick Access</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
          {[{icon:"🎬",label:"Cinema",p:"cinema"},{icon:"⚽",label:"Turf",p:"turf"},{icon:"🎮",label:"Games",p:"games"},{icon:"🍔",label:"Food",p:"food"}].map(q=>(
            <button key={q.label} onClick={()=>setPage(q.p)} style={{background:"#27272a",border:"none",borderRadius:14,padding:"14px 0",display:"flex",flexDirection:"column",alignItems:"center",gap:6,cursor:"pointer"}}>
              <span style={{fontSize:24,lineHeight:1}}>{q.icon}</span><span style={{color:"#9ca3af",fontSize:11}}>{q.label}</span>
            </button>
          ))}
        </div>
      </div>
      <div style={{padding:"0 16px 16px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          <span style={{color:"white",fontWeight:700,fontSize:16}}>The Arena</span>
          <button onClick={()=>setPage("turf")} style={{color:"#22c55e",fontSize:12,background:"none",border:"none",cursor:"pointer",fontWeight:600}}>Book Pitch</button>
        </div>
      ...
      <div style={{padding:"0 16px 16px"}}>
        <div onClick={()=>setPage("games")} style={{borderRadius:14,overflow:"hidden",position:"relative",height:140,background:"linear-gradient(135deg,#1a0520,#3d0d4e)",cursor:"pointer"}}>
          <div style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",fontSize:80,opacity:0.12}}>🎮</div>
          <div style={{position:"absolute",inset:0,background:"linear-gradient(to top,rgba(0,0,0,0.6),transparent)"}}>
            <div style={{position:"absolute",bottom:12,left:12}}>
              <div style={{color:"#ec4899",fontSize:10,fontWeight:700,marginBottom:2}}>🎮 GAME LOUNGE</div>
              <div style={{color:"white",fontWeight:700,fontSize:17}}>Virtual Reality Zone</div>
              <div style={{color:"#9ca3af",fontSize:12}}>PS5, VR, and Retro Arcade available.</div>
            </div>
          </div>
        </div>
      </div>
      <div style={{padding:"0 16px 16px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          <span style={{color:"white",fontWeight:700,fontSize:16}}>Canteen Deals</span>
          <button onClick={()=>setPage("food")} style={{color:"#FFD700",fontSize:12,background:"none",border:"none",cursor:"pointer",fontWeight:600}}>Order Now</button>
        </div>
        <div style={{display:"flex",gap:12,overflowX:"auto",paddingBottom:4}}>
          {canteen.map(c=>(
            <div key={c.name} style={{minWidth:150,background:"#18181b",border:"1px solid #27272a",borderRadius:14,overflow:"hidden",flexShrink:0}}>
              <div style={{height:90,background:`linear-gradient(135deg,${c.color}22,${c.color}44)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:40}}>
                {c.name.includes("Popcorn")?"🍿":c.name.includes("Burger")?"🍔":"🥤"}
              </div>
              <div style={{padding:"8px 10px"}}>
                <div style={{color:"white",fontWeight:700,fontSize:13}}>{c.name}</div>
                <div style={{color:"#9ca3af",fontSize:11,marginTop:2}}>{c.desc}</div>
                <div style={{color:"#FFD700",fontWeight:700,fontSize:13,marginTop:4}}>{c.price}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{padding:"0 16px 24px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          <span style={{color:"white",fontWeight:700,fontSize:16}}>Customer Reviews</span>
          <span onClick={onSeeAll} style={{color:"#FFD700",fontSize:12,cursor:"pointer",fontWeight:600}}>See All →</span>
        </div>
        {reviews.slice(0,3).map((r,i)=>(
          <div key={i} style={{background:"#18181b",border:"1px solid #27272a",borderRadius:14,padding:14,marginBottom:10}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <div style={{background:r.color,borderRadius:"50%",width:38,height:38,display:"flex",alignItems:"center",justifyContent:"center",color:"white",fontWeight:700,fontSize:13,flexShrink:0}}>{r.initials}</div>
                <div><div style={{color:"white",fontWeight:700,fontSize:14,marginBottom:3}}>{r.name}</div><Stars count={r.rating} size={13}/></div>
              </div>
              <CategoryBadge cat={r.category}/>
            </div>
            <div style={{color:"#9ca3af",fontSize:13,lineHeight:1.5,marginTop:4}}>{r.text}</div>
          </div>
        ))}
        <button onClick={onRateClick} style={{width:"100%",padding:"15px",background:"rgba(255,215,0,0.07)",border:"1px solid rgba(255,215,0,0.25)",borderRadius:14,color:"#FFD700",fontWeight:700,fontSize:15,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>⭐ Rate Your Experience</button>
      </div>
    </div>
  );
}
...
function CinemaPage({ setPage, setSelected }) {
  ...
  return (
    <div style={{paddingBottom:80}}>
      <div style={{padding:"16px",display:"flex",alignItems:"center",gap:12}}>
        <GKLogo size={36}/><div><div style={{color:"#9ca3af",fontSize:11}}>NOW SHOWING</div><div style={{color:"white",fontWeight:700,fontSize:20}}>Cinema</div></div>
      </div>
      <div style={{display:"flex",gap:8,padding:"0 16px",overflowX:"auto",paddingBottom:8}}>
        {days.map((d,i)=>(
          <button key={i} onClick={()=>setSelDay(i)} style={{minWidth:56,padding:"8px",borderRadius:12,border:"none",cursor:"pointer",background:selDay===i?"#FFD700":"#27272a",display:"flex",flexDirection:"column",alignItems:"center",flexShrink:0}}>
            <span style={{fontSize:9,color:selDay===i?"#000":"#9ca3af",fontWeight:600}}>{d[0]}</span>
            <span style={{fontSize:18,fontWeight:700,color:selDay===i?"#000":"white",lineHeight:1.2}}>{d[1]}</span>
            <span style={{fontSize:9,color:selDay===i?"#000":"#9ca3af"}}>{d[2]}</span>
          </button>
      ...
      <div style={{display:"flex",gap:8,padding:"12px 16px",overflowX:"auto"}}>
        {genres.map(g=><button key={g} onClick={()=>setSelGenre(g)} style={{padding:"6px 16px",borderRadius:20,border:"none",cursor:"pointer",background:selGenre===g?"#a855f7":"#27272a",color:selGenre===g?"white":"#9ca3af",fontWeight:600,fontSize:13,whiteSpace:"nowrap",flexShrink:0}}>{g}</button>)}
      </div>
      <div style={{padding:"0 16px"}}>
        {filtered.map(m=>(
          <div key={m.id} onClick={()=>{setSelected(m);setPage("movie");}} style={{background:"#18181b",border:"1px solid #27272a",borderRadius:16,marginBottom:14,overflow:"hidden",cursor:"pointer"}}>
            <div style={{display:"flex",gap:12,padding:12}}>
              <div style={{position:"relative",flexShrink:0}}>
                <div style={{width:80,height:110,borderRadius:10,background:m.gradient,display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",position:"relative"}}>
                  <span style={{fontSize:48,fontWeight:900,color:"rgba(255,255,255,0.15)",fontFamily:"serif"}}>{m.posterLetter}</span>
                  <div style={{position:"absolute",inset:0,background:"linear-gradient(to bottom,transparent 40%,rgba(0,0,0,0.5))"}}/>
                </div>
                <div style={{position:"absolute",top:4,left:4,background:m.badgeColor,color:"white",fontSize:8,fontWeight:700,padding:"2px 5px",borderRadius:4,lineHeight:1.3}}>{m.badge}</div>
              </div>
              <div style={{flex:1,minWidth:0}}>
...
export default function App() {
  ...
  return (
    <div style={{maxWidth:430,margin:"0 auto",background:"#09090b",minHeight:"100vh",fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif",color:"white",position:"relative",overflowX:"hidden"}}>
...
// (Copy everything from "const LOGO_URL..." 
// all the way down to the end of your file)
// ==========================================

const LOGO_URL = "https://i.postimg.cc/d0pZcWFK/Whats-App-Image-2026-02-20-at-11-20-53-PM.jpg";

function GKLogo({ size = 36 }) {
  // ... (the rest of your GKLogo code)
}

const movies = [ /* ... your movies data ... */ ];

// ... (Copy all your other components like CinemaPage, FoodPage, etc.)

export default function App() {
  const [page, setPage] = useState("home");
  // ... (Your entire main App function logic)
  return (
...
// ==========================================
// 2. THIS IS THE STARTER CODE (GLUE)
// This goes at the very, very bottom.
// ==========================================
